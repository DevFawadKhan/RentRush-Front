...
// filterdata
const Filterdata=invoices.filter(item =>item?.bookingId.toLowerCase().includes(searchTerm.toLowerCase()))
  return (
    <>
    <Navbar/>
    <div className="p-6 bg-gray-50 min-h-screen">
              ...
              {Filterdata.map((invoice) => (
                <tr key={invoice.bookingId} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full text-green-600`}>
                      pending
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      <button className="text-blue-600 hover:text-blue-900">
                        <FiEye onClick={()=>openPDF(invoice.invoiceUrl)} className="h-5 w-5" />
                      </button>
                      <button onClick={()=>handleDownload(invoice.invoiceUrl)} className="text-gray-600 hover:text-gray-900">
